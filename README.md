...
- `GET /flow` — air bridge flow snapshot: cumulative net eastbound tanker crossings
  (with all-time E/W audit), 7-day tanker flow, cargo 3-day rate vs 30-day baseline,
  per-gate counts over 30 days, and a 7-day series with each day's relay sample count.
  Gates and airframe classes are at the top of `server.js`; crossings persist in
  `flow-history.json` (45 days) and are deduped per airframe per gate with a 90-minute
  visit gap. The dashboard has no local fallback for this — it reports the record as
  unavailable when the service is asleep.
